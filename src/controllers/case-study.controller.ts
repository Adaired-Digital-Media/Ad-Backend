...
export const getCaseStudy = async (
  ...
  try {
    ...
    // Validate inputs for single case study query
    ...
    // Set query parameters for single case study
    if (id) {
    ...
    // Handle includeInactive: only filter by status if includeInactive is not true
    ...
    res.status(200).json({
      success: true,
      message,
      data: id || slug ? caseStudies : caseStudies, // Single object or array
    });
  } catch (error: any) {
